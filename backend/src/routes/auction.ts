...
/**
 * GET /api/auction/og/art/:mint
 * Serve HTML with Open Graph meta tags for art detail page link previews.
 */
router.get('/auction/og/art/:mint', async (req, res) => {
  try {
    const mint = req.params.mint;

    let title = 'CHUM: Reanimation';
    let imageUrl = 'https://www.clumcloud.com/chum-logo.jpg';
    let description = 'ASCII art NFT on Solana';

    try {
      const { data: candidate } = await supabase
        .from('art_candidates')
        .select('name, image_url, creator_wallet, votes')
        .eq('mint_address', mint)
        .single();
      if (candidate) {
        title = candidate.name || title;
        description = `${candidate.votes || 0} votes — ASCII art NFT on CHUM: Reanimation`;
        if (candidate.image_url) {
          const encoded = encodeURIComponent(candidate.image_url);
          imageUrl = `https://chum-production.up.railway.app/api/auction/og/image?url=${encoded}`;
        }
      }
    } catch (_) {}

    const url = `https://www.clumcloud.com/art/${mint}`;
...
<meta http-equiv="refresh" content="0;url=${url}"/>
    ...
    res.redirect(`https://www.clumcloud.com/art/${req.params.mint}`);
  }
});
