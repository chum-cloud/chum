#!/usr/bin/env python3
"""Generate ASCII art for ALL Hot Head NFTs (by creator J2AQy...).
Uses searchAssets since Hot Heads have no on-chain collection."""
...
CREATOR = "CywHUY59AFi7nmGf9kVfNgd39TD9rnkyx6GfWsn5iNnE"
...
def fetch_all_hotheads():
    url = f"https://mainnet.helius-rpc.com/?api-key={HELIUS_API_KEY}"
    all_items = []
    page = 1
    while True:
        payload = json.dumps({
            "jsonrpc": "2.0", "id": "hotheads",
            "method": "searchAssets",
...
def download_image(url, dest, timeout=10):
    ...
    # Skip broken IPFS gateways entirely
    if 'bafkrei' in url and 'cloudflare-ipfs' not in url and 'arweave' not in url:
        raise Exception('IPFS gateway unreliable, skipping')
...
def generate_one(args):
    ...
    prefix = f"hothead-{asset_id[:8]}-{int(time.time())}"
...
def main():
    ...
    print("Fetching all Hot Head NFTs...")
    items = fetch_all_hotheads()
    print(f"Found {len(items)} Hot Heads")
